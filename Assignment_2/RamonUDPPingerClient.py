...
class addRTT(elapsedTime, startTime):
    if elapsedTime == 0:
        elapsedTime = (time.time() - startTime)
        total = elapsedTime
    else:
        elapsedTimeHolder = elapsedTime
        elapsedTime = (time.time() - startTime)
        if elapsedTime < elapsedTimeHolder:
            rttMin = elapsedTime
            rttMax = elapsedTimeHolder
            total += total
        else:
            rttMin = elapsedTimeHolder
            rttMax = elaspedTime
            total += total
        average = float(total) / float(seqCount)
...
while seqCount<= 10:
    ...
    try:
        ...
        #RTT roundTripTime adding
        addRTT(elapsedTime, startTime)


        print('Ping message number ' + str(seqCount) + ' Round Trip Time(RTT): '
              + str(elapsedTime) + ' secs')
